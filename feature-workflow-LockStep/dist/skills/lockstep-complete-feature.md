...
基于 **AILock-Step 协议** 的 feature 宔成流程。
...
STP-003:
  ...
  !! OP_FS_READ("features/active-{REG_FEATURE_ID}/.status") >> REG_FEATURE_STATUS
  -> STP-010
...
STP-120:
  desc: "创建归档目录"
  !! OP_BASH("mkdir -p features/archive/done-{REG_FEATURE_ID}-{REG_DATE}") >> REG_ARCHIVE_DIR
  -> STP-121

STP-121:
  desc: "复制需求文档"
  !! OP_BASH("cp features/active-{REG_FEATURE_ID}/spec.md features/archive/done-{REG_ARCHIVE_DIR}/spec.md")
  -> STP-122
STP-122:
  desc: "复制任务文档"
  !! OP_BASH("cp features/active-{REG_FEATURE_ID}/task.md features/archive/done-{REG_ARCHIVE_DIR}/task.md")
  -> STP-123

STP-123:
  desc: "复制检查清单"
  !! OP_BASH("cp features/active-{REG_FEATURE_ID}/checklist.md features/archive/done-{REG_ARCHIVE_DIR}/checklist.md")
            # 无 checklist
            -> STP-124
            # 有
            -> STP-123
STP-124:
  desc: "复制 evidence 目录"
  !! OP_BASH("test -d features/active-{REG_FEATURE_ID}/evidence features/archive/done-{REG_ARCHIVE_DIR}/evidence/")
            # 无 evidence
            -> STP-125
            # 有
            -> STP-124
STP-125:
  ...
  !! OP_FS_WRITE("features/archive/done-{REG_FEATURE_ID}/archive-meta.yaml", REG_META_CONTENT)
  -> STP-126
STP-126:
  desc: "更新归档日志"
  !! OP_FS_READ("features/archive/archive-log.yaml") >> REG_ARCHIVE_LOG
...
STP-132:
  desc: "删除 active 目录"
  !! OP_BASH("rm -rf features/active-{REG_FEATURE_ID}")
  !! OP_FS_EXISTS("features/active-{REG_FEATURE_ID}")
  -> STP-ERR-DIR-CLEANUP
            # 成功
            -> STP-133
STP-133:
...
completion:
  ...
  worktree: ../OA_Tool-feat-auth (已删除)
